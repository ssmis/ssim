...
class ProjUnet2D(BaseUnet2D):
    ...
    def forward(self, inputs, output_final_feat=False):
        # ProjUnet2D forward pass
        center, conv4, conv3, conv2, conv1 = self.encoder(inputs)
        up1 = self.decoder(center, conv4, conv3, conv2, conv1)
        final = self.final(up1)

        result = {"seg_final": final}
        if output_final_feat:
            result["proj_final"] = self.proj_final(up1)
        return result


def unet_2D(method, cfg):
    # Factory method to create specific U-Net model
    model_dict = {"Base": BaseUnet2D, "Proj": ProjUnet2D}
    return model_dict[method](n_classes=cfg.DATA.SEG_CLASSES, feature_scale=cfg.MODEL.FEATURE_SCALE,
