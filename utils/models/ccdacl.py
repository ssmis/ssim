...
class conv_block_DCB(nn.Module):
    def __init__(self, ch_in, ch_out):
        super(conv_block_DCB, self).__init__()
        self.conv = nn.Sequential(
            DCB(ch_in, ch_out, kernel_size=3, stride=1, padding=1),

            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),

            DCB(ch_out, ch_out, kernel_size=3, stride=1, padding=1),
...
class up_conv_DCB(nn.Module):
    def __init__(self, ch_in, ch_out):
        super(up_conv_DCB, self).__init__()
        self.up = nn.Sequential(
            nn.Upsample(scale_factor=2),

            DCB(ch_in, ch_out, kernel_size=1),
...
class CCDACL(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=1,
        ...
        print("Transformer-embed_dim:{}; num_heads:{}; depths:{}".format(self.embed_dim, num_heads, depths))
        ...
        self.Conv2e = conv_block_DCB(ch_in=self.embed_dim * 1, ch_out=self.embed_dim * 2)
        self.Conv3e = conv_block_DCB(ch_in=self.embed_dim * 2, ch_out=self.embed_dim * 4)
        self.Conv4e = conv_block_DCB(ch_in=self.embed_dim * 4, ch_out=self.embed_dim * 8)
        self.Up4d = up_conv_DCB(ch_in=self.embed_dim * 8, ch_out=self.embed_dim * 4)
        self.Up_conv4d = conv_block(ch_in=self.embed_dim * 8, ch_out=self.embed_dim * 4)
        self.Up3d = up_conv_DCB(ch_in=self.embed_dim * 4, ch_out=self.embed_dim * 2)
        self.Up_conv3d = conv_block(ch_in=self.embed_dim * 4, ch_out=self.embed_dim * 2)
        self.Up2d = up_conv_DCB(ch_in=self.embed_dim * 2, ch_out=self.embed_dim * 1)
        ...
        self.CcdaCL_Conv = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):   # 1,3,224,224
        ...
        CNN = self.BN(CNN)
        SWIN = self.BN(SWIN)
        CcdaCL = torch.cat((CNN, SWIN), dim=1)
        CcdaCL = self.CcdaCL_Conv(CcdaCL)

        CcdaCL = torch.sigmoid(CcdaCL)
        CNN_out = torch.sigmoid(CNN_out)
        Trans_out = torch.sigmoid(Trans_out)

        return CcdaCL, CNN_out, Trans_out
