...
def train_ccdacl(_print, cfg, model, train_loader, valid_loader, criterion, valid_criterion, optimizer,
    ...
    labeled_idxes, unlabeled_idxes = set(), set()

    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        _print(f"Epoch {epoch + 1}")

        # Define metrics
        losses, losses_sup, losses_cont_g, losses_gc = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
        ...
        for i, batch in enumerate(tbar):
            ...
            image, target, p_image = image.to(device='cuda', dtype=torch.float), target.to(device='cuda', dtype=torch.float)[:cfg.TRAIN.LB_BATCH_SIZE], p_image.to(device='cuda', dtype=torch.float)

            # Model forward
            outputs = model(image, output_final_feat=True)
            output_target, proj_final = outputs['seg_final'], outputs["proj_final"]

            # Process patches
            cn, cs = cfg.MODEL.PROJECT_NUM, cfg.DATA.SIZE // cfg.MODEL.PROJECT_NUM
            p_image = p_image.unfold(2, cs, cs).unfold(3, cs, cs).permute(0, 2, 3, 1, 4, 5).reshape(-1, cfg.DATA.INP_CHANNELS, cs, cs)
            p_output = model(p_image, output_final_feat=True)
            p_proj_final, p_output_target = p_output["proj_final"], p_output['seg_final']
            p_proj_final = p_proj_final.reshape(bs, cn, cn, 512 // cfg.MODEL.FEATURE_SCALE, 1, 1).permute(0, 3, 1, 4, 2, 5).reshape(bs, 512 // cfg.MODEL.FEATURE_SCALE, cn, cn)

            # Calculate losses
            loss_sup = criterion_res(output_target[:cfg.TRAIN.LB_BATCH_SIZE], target[:cfg.TRAIN.LB_BATCH_SIZE])
            global_cont_loss = cont_loss(proj_final, p_proj_final) if epoch < cfg.OPT.WARM_UP else torch.tensor(0.0).cuda()
            global_consistency_loss = cons_loss(p_output_target, output_target.detach()) if epoch >= cfg.OPT.WARM_UP else torch.tensor(0.0).cuda()

            consistency_weight = cfg.OPT.MAX_C
            loss = (consistency_weight * global_cont_loss + loss_sup) if epoch < cfg.OPT.WARM_UP else (loss_sup + consistency_weight * global_consistency_loss)
    ...
    test_model(_print, cfg, model, test_loader, weight=os.path.join(cfg.DIRS.WEIGHTS, f"best_{cfg.EXP}_{cfg.MODEL.NAME}_fold{cfg.TRAIN.FOLD}.pth"))

    if not cfg.DEBUG:
        # Export stats to JSON
        tb.export_scalars_to_json(os.path.join(cfg.DIRS.OUTPUTS, f"{cfg.EXP}_{cfg.MODEL.NAME}_{cfg.COMMENT}_{round(best_metric, 4)}.json"))
        # Close Tensorboard
        tb.close()
