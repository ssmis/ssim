...
def get_loss(cfg):
    """
    Returns a list of loss functions as specified in the configuration.
    """
    loss = []
    ...
    if len(loss) == 0:
        raise Exception("Choose a loss function")
    return loss
...
class FinalConLoss(torch.nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        feat_q = F.normalize(feat_q.view(batch_size, dim, -1).permute(0, 2, 1), dim=-1, p=1)
        feat_k = F.normalize(feat_k.view(batch_size, dim, -1).permute(0, 2, 1), dim=-1, p=1).detach()

        # Positive logit
        l_pos = torch.bmm(feat_q.reshape(-1, 1, dim), feat_k.reshape(-1, dim, 1)).view(-1, 1)

        # Negative logit
        batch_dim_for_bmm = 1 if self.nce_includes_all_negatives_from_minibatch else batch_size
...
class FinalConsistLoss(nn.Module):
    ...
    def forward(self, patch_outputs, output):
        bs, cls, psz, cn = output.shape[0], output.shape[1], patch_outputs.shape[-1], output.shape[-1] // patch_outputs.shape[-1]
